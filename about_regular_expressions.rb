...
class AboutRegularExpressions < Neo::Koan
  ...
  #
  # Why?
  # They only take the first match they find (leftmost)
  ...
  # match anything in the brackets 1 or more times sequentially
  # match all numbers 0-9 one or more times
  ...
  # Stuff inside brackets is automatically escaped
  ...
  def test_period_is_a_shortcut_for_any_non_newline_character
    ...
    # Match a, then any non newline character any number of times
  end

  def test_a_character_class_can_be_negated
    assert_equal "the number is ", "the number is 42"[/[^0-9]+/]
    # match anything no a number an number of times
  end
  ...
  def test_slash_a_anchors_to_the_start_of_the_string
    ...
    # To match: the start of the string and then the word "start"
  end

  def test_slash_z_anchors_to_the_end_of_the_string
    assert_equal "end", "start end"[/end\z/]
    assert_equal nil, "start end"[/start\z/]
    # the word "end" and then the end of the string
  end
  ...
  def test_variables_can_also_be_used_to_access_captures
    # match a word then a comma and then another word
    # the parenthesis are only for recapturing later
    # can also be written as /\w+ \w+/
    assert_equal "Gray, James", "Name:  Gray, James"[/(\w+), (\w+)/]
    assert_equal "Gray", $1        # comma not included because it was explicitly in regex
  ...
  def test_a_vertical_pipe_means_or
    ...
    # below matches with 1 because counting starts at 1 with regex () capture
  ...
  def test_scan_is_like_find_all
    # scan finds ALL matches in the string and puts then in an array
    assert_equal ["one", "two", "three"], "one two-three".scan(/\w+/)
  end

  def test_sub_is_like_find_and_replace
    assert_equal "one t-three", "one two-three".sub(/(t\w*)/) { $1[0, 1] }
    # find the first word that starts with t and replace it with it's first character
  end
